...
export const App: React.FC = () => {
  ...
  const handleReorder = (reorderedItems: Station[]) => {
    ...
    if (playlistFilter === 'all') {
      ...
      // If we are reordering in "Favorites" filter
      const newStations = [...stations];
      let favIdx = 0;
      for(let i=0; i<newStations.length; i++) {
        if (favorites.includes(newStations[i].id)) {
          newStations[i] = reorderedItems[favIdx++];
        }
      }
    ...
    <div className="flex flex-col overflow-hidden transition-colors duration-500" style={{ height: 'var(--tg-viewport-height, 100vh)', color: nativeTextColor, backgroundColor: nativeBgColor }}>
      ...
      <div className="flex items-center justify-between px-6 bg-white/70 dark:bg-black/30 border-b border-black/5 dark:border-white/10 z-20 shrink-0 backdrop-blur-[70px]" style={{ paddingTop: isMobile ? 'calc(var(--tg-safe-top, 0px) + 46px)' : 'calc(var(--tg-safe-top, 0px) + 16px)', paddingBottom: '12px' }}>
        ...
        <div className="flex items-center gap-1">
          ...
          <RippleButton 
            onClick={() => { setShowPlaylist(true); setIsPlaylistEditMode(false); }} 
              ...
              <div className="w-full flex items-center justify-between px-8 pt-7 pb-3 shrink-0 touch-none">
                ...
                <div className="w-32 text-right">
                  <RippleButton 
                    ...
                    className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all active:scale-95 shadow-sm border border-transparent dark:border-white/5 focus:outline-none focus:ring-0 focus-visible:ring-0 ${isPlaylistEditMode ? 'text-white' : 'bg-black/5 dark:bg-white/10 text-gray-500 dark:text-gray-400'}`}
                    style={{ 
        ...
        {showAboutModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 30 }} className="relative w-full max-w-sm bg-white/95 dark:bg-black/85 rounded-[3.5rem] p-10 flex flex-col border border-white/20 dark:border-white/10 shadow-2xl backdrop-blur-[70px]">
              ...
              <div className="text-[13px] font-bold opacity-60 text-center mb-8 px-6 leading-relaxed">Премиальный плеер с двумя режимами плейлиста: навигация и управление.</div>
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 30 }} className="relative w-full max-w-sm bg-white/95 dark:bg-black/85 rounded-[3.5rem] p-10 shadow-none my-auto border border-white/20 dark:border-white/10 shadow-2xl backdrop-blur-[70px]">
              ...
              <form onSubmit={addOrUpdateStation} className="flex flex-col gap-4">
                ...
                <div className="flex gap-4 mt-6">
                  <RippleButton type="button" onClick={() => { setShowEditor(false); if (editingStation) setIsPlaylistEditMode(true); }} className="flex-1 py-4 bg-black/5 dark:bg-white/5 opacity-60 rounded-2xl font-black transition-all active:scale-95 shadow-sm focus:outline-none focus:ring-0 focus-visible:ring-0">Отмена</RippleButton>
